...
export default function ViewClients() {
  ...
  const fetchClients = async () => {
    try {
      const response = await axios.get('http://localhost:1011/viewclients');
  ...
  return (
    <div align="center" style={{ paddingTop: '20px',color: 'black' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <table border={1} id="myTable" align="center" style={{ width: 'auto', height: 'auto' }}>
          <thead>
            <tr>
            ...
            {Array.isArray(clients) && clients.length > 0 ? (
              clients.map((client, index) => (
                <tr key={index}>
                  ...
                  <td><button onClick={() => deleteclients(client.email)} className='button'>Delete</button></td>
                </tr>
              ))
            ) : (
              <tr>
